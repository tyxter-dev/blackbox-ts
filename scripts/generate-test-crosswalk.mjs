...
const pythonCompatibility = parentTests.map((pythonTest) => {
  ...
  const declined =
    explicit !== undefined &&
    explicit.typescript.size === 0 &&
    explicit.refs.every((ref) =>
      requirements.some(
        (requirement) =>
          requirement.disposition === 'declined' && requirement.evidence.includes(ref),
      ),
    );
  return {
    ...
    typescript_tests: explicit === undefined ? inferred.tests : [...explicit.typescript].sort(),
    notes: [
      ...(DIVERGENCE_NOTES[pythonTest] ?? []),
      ...(declined
        ? requirements
            .filter(
              (requirement) =>
                requirement.disposition === 'declined' &&
                requirement.evidence.some((ref) => explicit.refs.includes(ref)),
            )
            .map((requirement) => `Declined adoption: ${requirement.reason}`)
        : []),
    ],
  };
});
...
if (process.argv.includes('--check')) {
  ...
  if (current !== rendered) {
    throw new Error(
      'docs/parity-test-crosswalk.json is stale. Run pnpm generate:parity:crosswalk.',
    );
  }
  console.log(
...
function inferMapping(path) {
  const rules = [
    ...
    [
      /codex/,
      ['Agent Sessions'],
      ['tests/unit/codex-agent-provider.test.ts', 'tests/golden/codex-app-server-events.test.ts'],
    ],
    ...
    [
      /permission/,
      ['Skills and Workspace Agents', 'Local Tools', 'MCP'],
      [
    ...
    [
      /store|persist|resume|serializ/,
      ['Persistence', 'Persistence Contracts'],
      ['tests/unit/persistence-observability.test.ts', 'tests/golden/core-contracts.test.ts'],
    ],
    ...
    [
      /approval|policy|guardrail|safety/,
    ...
    [
      ...
      ['tests/golden/core-contracts.test.ts', 'tests/unit/core-contracts.test.ts'],
    ],
    [
      /runtime/,
      ['High-Level Runtime', 'Runtime Lifecycle Hardening'],
...
function domainForEvidence(ref) {
  return requirements.find((requirement) => requirement.evidence.includes(ref))?.domain;
}
