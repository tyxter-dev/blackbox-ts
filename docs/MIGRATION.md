...
The pinned Python ZIP fixture is generated by `scripts/python/generate_workspace_agent_fixture.py`
using the recorded Python checkout on `PYTHONPATH`, followed by
`pnpm exec prettier --write tests/fixtures/python/workspace-agent-package.json`. CI consumes
the checked-in fixture without Python. Other recorded differences remain in the generated [test crosswalk](parity-test-crosswalk.json).
