...
| Python 3.13 local vs 3.11 CI byte-parity of regenerated fixtures     | `parity:python` byte-compare fails only in one environment | Regenerate under Python 3.11 (preflighted 2026-09-06: `uv` 0.10.10 at `~/.local/bin/uv`, not on PATH; `uv python install 3.11` done → `PYTHON=$(~/.local/bin/uv python find 3.11)` is honoured by scripts/generate-python-fixtures.mjs:24); B1 regenerates under 3.11 by default and records the interpreter |
...
| R15 | A5      | ⇢ Spec shape for the grant model, all additive and optional so existing specs, SQLite rows, packages, and fixtures are untouched: `WorkspaceAgentSpec.permission_mode?: 'inherit' \| 'allowlist_v1'` (undefined ≡ inherit) and `WorkspaceAgentSpec.grants?: readonly WorkspaceAgentToolPermission[]` — the parent's `permissions: list[ToolPermission]` lands under the NEW name `grants` because TS `permissions: WorkspaceAgentPermissions` (src/workspace-agents/types.ts:11-18) already exists with different membership semantics; `WorkspaceAgentConnector` gains optional `tool_refs?: readonly string[]` and `auth_mode?: 'end_user' \| 'agent_owned'` (the parent's ConnectorSpec fields the connector-binds-ref check reads). `permission_mode` is validated in `validateWorkspaceAgent` (no constructor exists in TS), and `toAgentSpec` maps onto the existing TS `AgentSpec` fields only. New exports mirror parent names in camelCase (`compilePackagePermissions`, `toAgentSpec`, `canonicalRef`, `PackagePermissions`, `ToolGrant` …); `pnpm generate:api` is regenerated in-section for those parent-mirroring names only (CONTRACT DECISION covers them). MCP tool dispatch does not pass through `ToolRuntime` (mapper-verified) — the MCP enforcement point stays A7's per SIBLING SURFACES; the "hosted tool runner / dynamic toolset session" names in A6 did not resolve under those names in src/runtime (mapper) — A6's aggregation must re-anchor them | Field-name collision is a TS-only fact the parent could not anticipate; additive optional fields are the only shape that keeps R2's "default inherit changes nothing" provable from the type alone |
| R16 | A6      | ⇢ A6 enters no boundary and adds no request field: the runtime honors whatever boundary is already active (`activePermissions()`), exactly as the parent's main.py reads its contextvar — A7's `runWorkspaceAgent` bridge is the only entry point (R11). Per A5 corrections (i)–(iii): the parent's six spine sites map onto (1) `ModelRuntime.stream` per-turn `validatePackageModelConfig` before the capability assertion plus `tool_search` rejection under a boundary, (2) the agent-loop tool assembly + `refreshSelectedNames`/`toProviderTools` exposure filter via `definitionAllowed` with an explicit finalizer exemption (name check against `finalizerName`, since the finalizer has no handler identity to mark), (3) `ToolsetRuntime.search/load/allDefinitions/visibleDefinitions` filtering with `denied_by_package` semantics at `before_tool_exposure`, (4) the agent-loop and realtime per-call flow: `packageDecision` first, user policy only tightens, approval key recorded into `runtime.package_approvals` BEFORE awaiting the ticket, `TOOL_CHOICE_REJECTED` emitted from a `denied_by_policy` result, (5) `permissionBoundaryIterator` around the loop/session generators that a consumer drives (`AgentLoop.stream`, `AgentSessionsRuntime.stream`/`LocalAgentProvider.streamEvents` — wrap at ONE layer), and (6) no hosted execution gate (none exists). The handler-less-definition approval key (A5 capacity note 5) is pinned by an A6 test. | Adding a public request field is a floor item no ruling asked for and the parent does not need one; the collapse of six sites onto four TS surfaces is a structural fact of the tree, not a scope cut |
| R16a | A6      | ⇢ AMENDED at A6 review: the run-stream boundary wrap lives on the innermost public seam `AgentLoop.stream` (AgentRuntime.loop is a public readonly field and AgentLoop is exported, so `runtime.loop.stream()` must keep the boundary); `AgentRuntime.stream` is left unwrapped because `streamResolved` pulls the loop's wrapped generator from inside its own body. Agent-session and realtime wraps stay at `AgentSessionsRuntime.stream` and `ManagedRealtimeSession.events`. Also ruled: TOOL_CHOICE_REJECTED emission is guarded on enforcement (R2 strictness); the approval key is computed for ANY require_approval verdict under a boundary (parent agent_loop.py L418) so a user-policy-only approval opens the approved-call context A7's MCP nested checkpoint reads | A consumer-reachable public seam left unwrapped is a real bypass once A7 enters boundaries; the other two are parent-faithfulness fixes the security and contract lenses found |
| R17 | A7      | ⇢ A7 shape, per its mappers: (1) `supports_package_permissions` is added to the agent-provider `AgentCapabilities` in src/providers/agent.ts (NOT src/core/capabilities.ts, which is the model/turn profile) — LocalAgentProvider true; InjectedCloudAgentProvider forces false regardless of the injected client; Vertex literal false; (2) the facade gate (parent agents.py L107/L186) lands in `AgentSessionsRuntime.createAgent/start/stream` using the existing capability-gate idiom; (3) LocalAgentProvider snapshots `activePermissions()` per agent at createAgent (after `validatePackageModelConfig(spec.hosted_tools ?? [])`) and composes per-session `[...agent, ...active]` at startSession, storing them on its records; it does NOT add a third generator wrap — the parent wraps in the local adapter only to re-enter its stored snapshots, so the TS equivalent is: `LocalAgentProvider.streamEvents` re-enters `[...activePermissions(), ...stored]` around each step via `permissionBoundaryIterator` ONLY when stored snapshots are non-empty (otherwise the facade wrap already covers it); (4) the bridge is `runWorkspaceAgent(runtime, spec, options)` in a new src/workspace-agents/runtime.ts (parent-mirroring name; exported; API snapshot regenerated): compile grants for allowlist_v1 → preflight fail-before-side-effect (adapter capability when `spec.agent_provider` is set; MCP/workspace support checks only where TS has the corresponding capability flag — otherwise recorded as N/A) → `permissionBoundary` → lower via the private inherit path and run through `runtime.run`/`stream`; WorkspaceAgentSpec.mcp_servers are bare names with no TS resolver, so MCP connections and workspace come from the caller's run options (documented; no invented resolution); (5) MCP and workspace metadata producers per a7-mapper-mcp-workspaces: workspace tools gain `category: 'workspace'`, `metadata.workspace_operation`/`ref` and the parent's canonical scopes table; `mcpToolDefinitions` copies descriptor metadata and derives scopes via the parent ladder (explicit permission_scopes → delete if destructive → read if read_only → execute); the MCP call-site pin (parent connector.py L211-234) lands at the ONE TS site that executes MCP-registered handlers outside ToolRuntime — the in-process `MCPServer` tools/call branch (src/mcp/server.ts:77-81) — plus `MCPClient.callTool` before dispatch: package decision on the fresh descriptor, approved-call check, TOOL_CHOICE_REJECTED, identity pin with the parent's retry message; `LocalWorkspace.authorize`/`WorkspaceRuntime.open` Policy checks stay as they are (separate user-policy mechanism, parent-equivalent) | The mapper-corrected facts (capability type location; no client hosted tools; bare mcp_servers) make the parent's exact shape impossible to copy; these calls keep every parent invariant reachable in TS without inventing surface beyond parent-mirroring names |
| R17a | A7      | ⇢ AMENDED at A7 review: R17(4)'s "when spec.agent_provider is set" cannot hold because TS `WorkspaceAgentSpec` has no such field (R15 froze the spec). The parent's `agent_provider` relocates to `options.agent_provider` on the exported `WorkspaceAgentRunOptions`, which is the typed mirror of the parent's `**kwargs` (same names, same semantics) — ratified as parent-mirroring in substance, not a TS-only surface; on that path, caller `model`/`instructions` overrides must be honoured exactly as on the model path (contract-lens finding). Portability consequence recorded for B1/C3 with D6: a parent package's `agent_provider`/`agent_id`/`model_provider`/`hosted_tools`/`extra` ride through import as ignored stray fields; `agent_id` (resume an existing agent) is inexpressible in TS; `metadata.run_request` is a TS-only local-adapter convention | Relocating rather than widening the spec keeps R15; the option is the parent's own field under the parent's own name |
| R18 | A9      | ⇢ A9 shape per its mapper: (1) Claude turn/task-budget — the TS `ClaudeCodeAgentProvider` is an injected-client wrapper that emits no events and owns no query loop, so the parent's per-turn `MODEL_REQUEST_STARTED {model, turn}` emission, model precedence, process startup/reap, and SDK loop are honestly N/A (recorded for B1's inventory); what ports is `_normalize_task_budget`: `ClaudeCodeAgentProvider.startSession` (and `sendMessage` if a budget can arrive there) validates `task.metadata.task_budget` when present — exactly `{ total: <integer 1..1_000_000> }` (reject floats, booleans, 0, >1_000_000, non-object, extra keys) — throwing ConfigurationError before the injected client is called; absent budget → untouched (R2); (2) MCP `clientInfo`/`serverInfo` version literals move to `0.2.0`, names unchanged (R6); no fixture covers the old string (mapper-verified) so no expected-red case; (3) the skills fallback parser gains multi-line list items: a line whose trimmed content is `-` (empty item) or starts with `- ` continues the preceding key's array, mirroring the parent's `_is_list_item` and its call sites, including list-of-maps items (`- key: value` continuation lines) as far as the parent's fallback supports them; `skillToMarkdown` stays as is (it never emits block lists) and the round-trip case parses a hand-written block-list SKILL.md then re-serialises and re-parses it | Mapper-verified absences make the parent's exact turn contract unportable; a boundary validator keeps the budget contract's rejection semantics; the parser change is the only genuinely new grammar |
...
- **Python-version byte drift** — B1 — local Python 3.13 vs CI 3.11 could produce differing fixture bytes — mitigation in Known blockers (regenerate under 3.11 via uv when detected); B1 and the final local gate re-prove at 3.11. C1 removes Python from CI per F3.
...
- 2026-09-07 — C3 mapper — tests have six executable suite directories plus fixtures (seven top-level directories); Anthropic Managed Agents is the partial/beta work source in src/workers/index.ts, not a cloud agent provider; the crosswalk is docs/parity-test-crosswalk.json (there is no docs/test-crosswalk.md). The mapper's proposed repeated focused suites are unnecessary for prose-only C3; preserve the scheduled final gate.
...
- 2026-09-06 — A5 — (i) TS has no client-executed hosted tools: every adapter emits provider-native wire objects (OpenAI mapHostedTool, Anthropic hosted loop, Gemini googleSearch/raw, xAI unsupported), so the parent's hosted_runtime.py execution-time package→user→fresh-package gate has NO TS counterpart — only the config gate (`validatePackageModelConfig`, currently unwired) applies; (ii) TS has no tool-routing module, hosted runtime, or toolset-session file: `ToolsetRuntime` (src/tools/catalog.ts) plus `ToolRegistry.toProviderTools` are the entire routing/exposure surface, so A6's "routing candidates" and "hosted runner" sites collapse onto those; (iii) the parent's spine sites have zero TS counterparts today — no package∘user policy composition, no `package_approvals` writer, no `tool_search` rejection, no exposure filtering, and the finalizer tool is not marked exempt (it is only structurally exempt at dispatch because the loop consumes it before executable calls); (iv) ToolRuntime enforcement is gated by `activePermissions().length > 0 || package_approvals.size > 0` (inert path byte-identical); (v) workspace tool definitions (src/workspaces/tools.ts) carry neither `category: 'workspace'` nor `metadata.workspace_operation`, so parent-shaped `workspace:*` grants admit nothing until A7 adds them; (vi) MCP dispatch (src/mcp/client.ts, server.ts) bypasses ToolRuntime — A7's enforcement point; (vii) portable-package interchange: the parent stores grants under `permissions`, TS under `grants` (R15) — a parent-written allowlist_v1 package fails closed in TS (see D6); (viii) the parent-mirroring core module src/core/tool-permissions.ts is deliberately not root-exported (parent keeps core.tool_permissions out of __init__), and `markInternalDiscoveryTool` is a spoofable exemption contained only by that constraint — keep it non-exported in A6/A7.

- 2026-09-06 — A6 — (i) R16a's stated mechanism was wrong: a lazy `async *` facade reaches `loop.stream` only at first pull (consumer context), so the facade must CALL `AgentLoop.stream` eagerly in the caller's frame; done — `AgentRuntime.stream` now validates the request synchronously at call time (model_required at call, not first pull; no in-repo consumer affected) — record for B1 crosswalk. (ii) Divergences kept deliberately for R2, for B1's crosswalk notes: a package-denied loop call still emits TOOL_CALL_REQUESTED/STARTED and TOOL_CALL_FAILED before TOOL_CHOICE_REJECTED (parent short-circuits with only the rejection); user-policy deny / denied ticket still throw ApprovalError and end the run (parent continues with a failed item); an unregistered call name under a boundary ends as tool_not_found without TOOL_CHOICE_REJECTED (parent denies with the event); `load_tools` reports refused names as `denied`/`denied_by_package` on its payload with no per-load event (parent: `invalid` list + run-visible event). (iii) Hosted config is validated twice per turn by design (loop: user-policy before_hosted_tool_config + rejection event; ModelRuntime: `extra` check + non-loop callers) — do not delete the "duplicate". (iv) Realtime `connect` passes tool definitions and hosted_tools to the provider unfiltered (parent-faithful; parent realtime has no permission handling); dispatch still denies; hosted tools there have no dispatch backstop — A7 deferral line. (v) The approval key is recorded for ANY awaited approval under a boundary (parent agent_loop.py L418), so a user-policy-only approval opens the approved-call context — A7's MCP nested checkpoint relies on it. (vi) Inherited scope-default asymmetry surfaced by exposure: grants default to scopes ['read'] while scope-less tools request ['execute'] — parity with the parent, note for docs (C3). (vii) Private d.ts members changed names (AgentLoop.streamRun new; AgentRuntime.observe replaces streamResolved); public signatures identical; `ToolsetRuntime.load()` return widened void → readonly string[].

- 2026-09-06 — A7 — (i) workspace canonical refs diverge from the parent by construction of the pre-existing tool names: TS `workspace:read|list|write|command` vs parent `workspace:read_file|list_files|write_file|run_command`; a literal parent grant does not match a TS workspace tool (B1 crosswalk + C3 docs, folded into D6). (ii) `connector_scopes` for MCP tools derive from the descriptor's `required_scopes` only (the parent also unions the host trust blob's and `metadata.scopes`) — TS is the more permissive side; B1 note. (iii) `readTool` reads `read_only`/`destructive` only from a wire `metadata` field, never from standard MCP `annotations.readOnlyHint/destructiveHint`, so against a real server the scope ladder's middle rungs are unreachable — pre-existing gap, ruled into A9 (R18a). (iv) The descriptor `scopes` array is an extra ladder rung with no parent counterpart (documented in code). (v) `listTools` now coalesces in-flight fetches and preserves descriptor identity across unchanged refetches: concurrent cold callers emit one MCP_LIST_TOOLS pair instead of N (no test/consumer asserts per-caller counts); `refresh: true` joins an in-flight fetch (no first-party caller passes it); the coalesced fetch runs on the first caller's AbortSignal — CHANGELOG note at B1. (vi) The in-process `MCPServer` `tools/list` omits descriptor metadata, so a connector-bound definition on an in-process server cannot be admitted by any grant (fail-closed; no first-party producer writes it). (vii) The agent-provider path of `runWorkspaceAgent` carries tools/hosted tools/workspace/policy on `AgentSpec.metadata.run_request` and now also writes the effective model/instructions onto the spec — a TS-only local-adapter convention C3 documents. (viii) Pre-existing unrelated defect found by the cancellation case: cancelling a local session streamed through `AgentSessionsRuntime.run/stream` throws SessionTerminalError (cancelled → failed transition, src/runtime/agent-sessions.ts:264) — D7. (ix) Two facade notes: `AgentSessionsRuntime.stream` under a boundary resolves the provider eagerly (ProviderNotFoundError at call rather than SessionNotFoundError at first pull); any adapter not advertising `supports_package_permissions` (incl. test fakes) is refused under a boundary — intended.

- 2026-09-06 — A8 — (i) The TS Codex client boundary is connection-level (`connect` → `send`/`messages`/`close`) while the parent's `CodexAppServerClient` protocol is session-level; the provider owns thread/turn requests and the normalization table (R14a). (ii) Six type-only protocol shapes are root-exported with the interface (R14a). (iii) `supports_resume` is forced false as a TS addition (the parent forces only package permissions); `supports_follow_up` true is a TS-only flag; `metadata.codex_sdk_version` names the pinned protocol version. (iv) Deviations in the fail-closed direction, all documented in code: numeric JSON-RPC request ids are answered (parent ignores them); a duplicate pending approval id is refused with −32000 (parent overwrites); pending approvals are dropped on close AND on reader failure with an explicit unknown-approval error afterwards (parent orphans silently); after a reader fault the connection is write-closed and `cancel` skips the interrupt (the parent's net was `_write` raising on a dead pipe). (v) The buffered `blackbox/session/started` entry is unreachable by any cursor (parent quirk kept). (vi) `AgentSessionsRuntime.run` output for a Codex session is '' (TS derives text from SESSION_COMPLETED/AGENT_RESPONSE_MESSAGE_CREATED, not deltas) — B1 crosswalk note against the parent's collect test. (vii) Parent `permissions`/`extra` → TS `metadata` conventions (`AgentSpec.metadata.id/.sandbox`, `TaskSpec.metadata.ephemeral/.sandbox`, `WorkspaceSpec.metadata.root`); C3 documents. (viii) Honest N/A for B1's inventory: inherited-env stripping, bundled PATH, codex_bin/SDK version check, kill grace, stderr tail, the parent's include-normalization compat registry, and its integration test. (ix) The fake lives test-local under tests/fixtures (src/testing is root-exported public API).

- 2026-09-06 — A9 — (i) The Claude task-budget contract ports only `_normalize_task_budget`, at `ClaudeCodeAgentProvider.startSession` on `task.metadata.task_budget`; the parent's per-turn MODEL_REQUEST_STARTED emission, model precedence, process/SDK machinery, and the agent-level `permissions.task_budget` precedence are honestly N/A (the TS wrapper retains no AgentSpec, so only the task-level budget is validated); JS admits `1.0`/`1e6` as integers where the parent rejects floats. (ii) MCP `clientInfo`/`serverInfo` versions are `0.2.0` (parent-mirrored) while the package is 0.1.0-alpha.1 (R5 bumps to 0.2.0-alpha.0 at C3); the Codex app-server client sends `clientInfo.name: 'blackbox'` (parent-mirrored) while the MCP client sends `blackbox-ts` (R6) — R6 is MCP-specific, not repo-wide. (iii) The skills fallback parser now mirrors the parent's whole YAML-subset block grammar; parent-faithful edge-input changes: a bare `key:` yields null (was ''), leading spaces before a top-level key and a leading-colon line fail closed, a lone `\r` splits lines — no in-repo producer emits any (skillToMarkdown quotes every string); inline flow scalars (`[a, b]`, `{ k: v }`, `~`, single quotes) remain JSON-only — the parent's hand-written SKILL.md fixture still does not parse in TS (block grammar aligned only; C3 documents). (iv) MCP descriptor `annotations.readOnlyHint/destructiveHint` map to `metadata.read_only/destructive` (boolean only; explicit key wins; the parent's whole-object `annotations` copy is not ported).

- 2026-09-06 — B1 — (i) The parent has 118 test modules and 129 evidence files at d5be68e0 (plan estimates: 118 modules; evidence 109→129). (ii) Feature 144 lands as a Supported row in the skills-and-workspace-agents group; the inventory carries no per-feature description, so "the cloud-agents description gains CodexAgentProvider" landed as an evidence symbol. (iii) The R3 direction lock is `STATUS_RANK` Not supported yet < Contract only < Partial < Supported where advertised < Supported with `target ≥ parent`; unknown statuses are rejected. (iv) Divergences and honest N/As are carried as 15 crosswalk `notes` (new additive field, schema_version unchanged) keyed by parent test module, not as status changes; the explicit `/codex/` and `/permission/` crosswalk rules are dormant safety nets (every new parent module has direct evidence). (v) Python fixtures are byte-identical between 3.11.15 and 3.13.5 at this pin; committed bytes are from 3.11 to match CI. (vi) MCP: only `clientInfo.name` diverges (`blackbox-ts`); `serverInfo.name` is the constructor name exactly as in the parent — earlier plan wording that R6 covers both names was imprecise. (vii) Historical old-pin records (ADR headers, docs/PARITY_PLAN.md, the alpha.0 CHANGELOG entry) stay per R19; CHANGELOG carries a `## Unreleased` entry that C3 renames at the R5 bump. (viii) Queued for the final-review correction round: literal U+0000 separators in src/core/tool-permissions.ts:335 (see Graph Findings).
...
- [x] A3 anthropic adaptive-model controls — parent-mirrored control + replay cases pass — accepted 2026-09-05 `0451ac2` — rounds: 1 — review: independent ×5 (security, contract, failure-mode, convention, doc-truth; security ran adversarial probes that disproved a vacuity claim and found a crash regression) — routing: requested=gdi-implementer; fallback=general-purpose+opus — cost: ~1.1M tokens / 7 agents — env-retries: 0
...
- [x] A5 permission grants core — deny/admit dispatch pair proven in unit suite — accepted 2026-09-06 `7ad0674` — rounds: 2 — review: independent ×6 (security, contract, failure-mode, capacity/false-positive, convention, doc-truth; security and capacity lenses ran probes against dist that found the hosted-gate type hole and the inert-path regressions) — routing: requested=gdi-implementer/gdi-reviewer/gdi-convention-reviewer/gdi-mapper; confirmed — cost: ~965k tokens / 9 agents (2 mappers ~166k, implementer 3 turns ~249k, 6 reviewers ~550k) — env-retries: 0
  - R1 security: validatePackageModelConfig classified hosted specs by spec.type while every adapter spreads config last, so config.type could ship a provider-executed tool under a client-executed grant (probe-confirmed against dist); plus two orchestrator-verified hardenings from lens notes — requestScopes admitted a non-array scopes value as ['execute'] where the parent denies, and the parent's fresh_dispatch_after_approval invalidation case had no runtime mirror
  - R2 doc-truth: three comment over-claims (hostedKind "every adapter" false for Gemini; ToolGrant doc omitted the admin wildcard; permission_mode type doc promised enforcement A6 has not wired) plus two capacity-lens probe results on the no-boundary path R2 protects — approvalKey computed on every dispatch even when inert (~55x with large metadata) and the own-enumerable spread copy dropping prototype accessors of class-instance tool definitions
- [x] A6 permission enforcement spine — exposure/routing/dispatch gates enforce the boundary — accepted 2026-09-06 `c7b787e` — rounds: 1 — review: independent ×6 (security, contract, failure-mode, capacity/false-positive, convention, doc-truth; security/failure-mode/capacity lenses ran probes against the tree) — routing: requested=gdi-implementer/gdi-reviewer/gdi-convention-reviewer/gdi-mapper; confirmed — cost: ~1.05M tokens / 9 agents (2 mappers ~171k, implementer 2 turns ~323k, 6 reviewers ~627k) — env-retries: 0
  - R1 doc-truth: three spine test descriptions promised properties their assertions could not discriminate (partial-consumption close, cancellation teardown, realtime approval forgotten); plus orchestrator-upgraded lens notes — guard TOOL_CHOICE_REJECTED on enforcement (contract), compute the approval key for any require_approval verdict as the parent does (security; A7 nested-checkpoint seam), move the run-stream wrap to the innermost public seam AgentLoop.stream (contract; R16 amended), add the plain-grant admission and positive toolset-routing cases the capacity probes validated, type the finalizer-exemption parameter, and fix two comment over-claims
- [x] A7 permission metadata + providers — end-to-end grant journey and denial twin pass — accepted 2026-09-06 `256e390` — rounds: 1 — review: independent ×6 (security, contract, failure-mode, capacity/false-positive, convention, doc-truth; security/contract/failure-mode/capacity lenses ran probes against the tree; the two rejecting lenses' fixes were verified by the orchestrator against code, the new cases, and the implementer's mutation probes) — routing: requested=gdi-implementer/gdi-reviewer/gdi-convention-reviewer/gdi-mapper; mappers confirmed; implementer confirmed for the first turn; fallback=general-purpose+fable for all six reviewers and the correction turn (Opus session limit, ⚙ below) — cost: ~1.3M tokens / 11 agents (2 mappers ~175k, implementer ~290k + correction ~143k, 6 reviewers ~687k, 3 rate-limited reviewer starts) — env-retries: 1
  - R1 security: LocalAgentProvider.execute entered the session's stored snapshot instead of composing it with the caller's active boundary (probe: a narrower ambient boundary at sendMessage was dropped and a write executed); contract: on the agent-provider path options.model/instructions were silently dropped while the options doc promised they win; orchestrator-upgraded failure-mode note: uncoalesced listTools refreshes made the MCP identity pin fire spuriously on concurrent calls or a TTL straddle (parent never fires it spuriously); plus comment precision (execute "composes exactly as the parent", server-pin "post-policy re-resolution", the scopes-array ladder rung) — lost-agent: the Opus implementer was rate-limited, so the correction resumed with a fresh general-purpose+fable implementer given the full prior report
  - ⚙×1 2026-09-06 12:15 UTC: all three first-batch reviewers (gdi-reviewer → opus) terminated by the Opus session limit (HTTP 429, resets 07:30 UTC) before reporting; per the Known blockers precedent (A1) the lenses were re-dispatched as `general-purpose` + `fable` (fresh context, tier at or above the pinned Opus — not a reduction); the implementer's report and diff were unaffected
- [x] A8 codex agent provider — fake-client contract suite passes — accepted 2026-09-06 `1a5dcca` — rounds: 1 — review: independent ×5 (security, contract, failure-mode, convention, doc-truth; security and failure-mode lenses ran probes against the tree; ⚠ set minus capacity, which has no admission policy to govern here) — routing: requested=gdi-implementer/gdi-reviewer/gdi-convention-reviewer/gdi-mapper; mapper confirmed; fallback=general-purpose+fable for the implementer (both turns) and all five reviewers (Opus session limit) — cost: ~1.35M tokens / 7 agents (mapper ~79k, implementer ~260k + correction ~285k, reviewers ~525k) — env-retries: 0
  - R1 failure-mode: after a reader-loop failure the connection was never marked closed, so cancel() on a failed session issued turn/interrupt and awaited a reply that could not arrive (probe: cancel timed out) and approve() wrote to the dead transport; plus doc-truth: the session-started buffer entry was described as "reachable only by replay" though no cursor reaches it, four stale claim anchors, two parent citations off by a line
- [x] A9 small parity fixes — claude budget, mcp versions, skills round-trip pass — accepted 2026-09-06 `6ef34c8` — rounds: 0 — review: independent ×3 (contract, convention, doc-truth; the contract lens probed HEAD-vs-tree parser equivalence over every skillToMarkdown output) — routing: requested=gdi-implementer/gdi-reviewer/gdi-convention-reviewer/gdi-mapper; mapper confirmed; fallback=general-purpose+fable for both implementer turns and all three reviewers — cost: ~0.5M tokens / 6 agents (mapper ~64k, lost implementer ~? (rate-limited before reporting), resume ~102k, reviewers ~232k) — env-retries: 1 (⚙ below)
  - ⚙×1 2026-09-06 ~13:40 UTC: the A9 implementer (general-purpose+fable, itself the Opus-limit fallback) was terminated by the Fable session limit (HTTP 429, reset 17:30 UTC) after its source edits landed and before its report; per the lost-agent rule a new implementer resumed A9 from the surviving diff (7 files) at 20:10 UTC

## Phase B — baseline bump

- [x] B1 pin bump + regeneration + counts — pnpm check and parity:python green at d5be68e0 — accepted 2026-09-06 `50f6a9a` — rounds: 1 — review: independent ×4 (data, contract, doc-truth, convention; the data and contract lenses re-proved the R3 lock, the R4 fail-closed snapshot, and the 3.11 fixture regeneration on scratch mirrors) — routing: requested=gdi-implementer/gdi-reviewer/gdi-convention-reviewer; fallback=general-purpose+fable for the implementer (both turns) and all four reviewers (Opus session limit) — cost: ~0.8M tokens / 5 agents (implementer ~185k + correction ~199k, reviewers ~390k) — env-retries: 0 — gates: pnpm check (impl run 1 of 2) and parity:python (impl run 1 of 1) both green under Python 3.11.15, fixtures byte-identical under 3.13.5
  - R1 doc-truth: the crosswalk note for the MCP server module and CHANGELOG.md claimed serverInfo's name is blackbox-ts (it is the constructor name, parent-faithful; only clientInfo.name diverges under R6) — data and doc-truth lenses both rejected on it; plus a crosswalk mapping gap (the Codex-output note mapped to a suite with no Codex case) and one CHANGELOG clause (Codex supports_resume forced false)
...
Follow-up resolution — 2026-09-08: the descriptions below preserve the original findings.
D1 and D3–D8 are resolved in the TypeScript implementation, with focused evidence recorded
here; issue closure and whole-branch verification gates are separate. D2 remains external/open.

- D1: schema-v3 TypeScript score includes OpenRouter; native expectations and the TS-first
  crosswalk pass `pnpm check:parity` and `tests/unit/parity-maintenance.test.ts`. The canonical
  replay is `tests/golden/core-contracts.test.ts`; Python samples now run under `tests/compatibility`.
- D3–D5: `tests/unit/planning-accounting-config.test.ts` covers combined counters, exact-first
  aliases, independent rates and source URLs; canonical native pricing expectations add fixed
  component checks. Bundled creation-rate defaults are intentionally retained as documented
  in `docs/MIGRATION.md`.
- D6: `importPythonWorkspaceAgentPackage` translates a real pinned Python ZIP into a native
  spec and explicit run options; `tests/unit/workspace-agents.test.ts` and
  `tests/unit/workspace-agent-runtime.test.ts` prove model-loop/local execution and grants.
  Other agent selectors, ambiguous hosted dataclasses, MCP, skills, schedules and active
  memory/publication behavior fail explicitly. No reverse exporter is promised.
- D7: `tests/unit/agent-sessions.test.ts` and `tests/unit/permission-spine.test.ts` cover terminal
  cancellation, trailing diagnostics, partial consumption and durable replay.
- D8: `tests/unit/codex-agent-provider.test.ts`, `tests/unit/agent-sessions.test.ts` and
  `tests/golden/codex-app-server-events.test.ts` separate private wire diagnostics from native
  approvals and adapter-owned lifecycle authority.


| ID  | Class         | Remaining work and risk                                                                                                                                                                                               | Owner / issue    | Re-entry gate                                                            | Blocks | Status |
| --- | ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- | ------------------------------------------------------------------------ | ------ | ------ |
| D1  | product-scope | Full parity-scoring inversion (crosswalk direction, fixture goldenness swap, extension→declined-parent-feature semantics, TS-owned single score). Risk: the inventory vocabulary still reads Python-first until done. | maintainers / https://github.com/tyxter-dev/blackbox-ts/issues/2 | linked issue acceptance checklist                       | none   | resolved — 2026-09-08 |
