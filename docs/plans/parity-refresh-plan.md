...
- [x] A6 permission enforcement spine — exposure/routing/dispatch gates enforce the boundary — accepted 2026-09-06 (this commit) — rounds: 1 — review: independent ×6 (security, contract, failure-mode, capacity/false-positive, convention, doc-truth; security/failure-mode/capacity lenses ran probes against the tree) — routing: requested=gdi-implementer/gdi-reviewer/gdi-convention-reviewer/gdi-mapper; confirmed — cost: ~1.05M tokens / 9 agents (2 mappers ~171k, implementer 2 turns ~323k, 6 reviewers ~627k) — env-retries: 0
...
- [x] A8 codex agent provider — fake-client contract suite passes — accepted 2026-09-06 (this commit) — rounds: 1 — review: independent ×5 (security, contract, failure-mode, convention, doc-truth; security and failure-mode lenses ran probes against the tree; ⚠ set minus capacity, which has no admission policy to govern here) — routing: requested=gdi-implementer/gdi-reviewer/gdi-convention-reviewer/gdi-mapper; mapper confirmed; fallback=general-purpose+fable for the implementer (both turns) and all five reviewers (Opus session limit) — cost: ~1.35M tokens / 7 agents (mapper ~79k, implementer ~260k + correction ~285k, reviewers ~525k) — env-retries: 0
